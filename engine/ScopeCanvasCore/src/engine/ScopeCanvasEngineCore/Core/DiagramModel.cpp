...
void DiagramModel::removeNode(CanvasNodeId nodeId)
{
    ...
    for (CanvasConnectorId connectorId : connectors)
    {
        ...
        m_connectors.erase(
            std::remove_if(m_connectors.begin(), m_connectors.end(), [connectorId](const Connector& value) {
                return value.id == connectorId;
            }),
            m_connectors.end());
    }
    ...
    m_nodes.erase(std::remove_if(m_nodes.begin(), m_nodes.end(), [nodeId](const Node& value) { return value.id == nodeId; }),
                  m_nodes.end());
}
...
void DiagramModel::disconnect(CanvasEdgeId edgeId)
{
    ...
    m_edges.erase(std::remove_if(m_edges.begin(), m_edges.end(), [edgeId](const Edge& value) { return value.id == edgeId; }),
                  m_edges.end());
}
...
{
    const auto it = std::find_if(m_nodes.begin(), m_nodes.end(), [nodeId](const Node& node) { return node.id == nodeId; });
...
{
    const auto it = std::find_if(m_edges.begin(), m_edges.end(), [edgeId](const Edge& edge) { return edge.id == edgeId; });
