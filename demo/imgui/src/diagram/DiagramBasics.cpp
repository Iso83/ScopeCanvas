...
std::vector<glm::vec2> buildFallbackRoutePoints(const Core::Node& fromNode, std::size_t fromIndex,
                                                const Core::Node& toNode, std::size_t toIndex) {
    ...
    if (startNormal.x == endNormal.x) {
        return {start, startStub, {startStub.x, end.y}, end};
    }
