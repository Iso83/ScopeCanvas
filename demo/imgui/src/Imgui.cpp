...
using namespace ScopeCanvas::Demo::Common;
using namespace ScopeCanvas::Demo::ImGui;
    ...
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("Create")) {
            if (ImGui::MenuItem("Number"))
                drawCtx.document().createNode(NodeTypeId{1}, {-120.0F, 0.0F});

            if (ImGui::MenuItem("Add"))
                drawCtx.document().createNode(NodeTypeId{2}, {80.0F, 0.0F});

            if (ImGui::MenuItem("Multiply"))
                drawCtx.document().createNode(NodeTypeId{3}, {280.0F, 0.0F});

            if (ImGui::MenuItem("Output"))
                drawCtx.document().createNode(NodeTypeId{4}, {480.0F, 0.0F});
