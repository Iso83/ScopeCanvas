...
{
std::vector<Engine::Core::Vec2> EdgeRenderer::buildEdgeGeometry(const Engine::Routing::EdgeRoute& route,
                                                                 int segmentsPerCurve) const
{
    ...
    std::vector<Engine::Core::Vec2> geometry;
    geometry.reserve(route.points.size() * static_cast<std::size_t>(segments));

    for (std::size_t i = 0; i + 1 < route.points.size(); ++i)
    {
        const Engine::Core::Vec2 p0 = route.points[i];
        const Engine::Core::Vec2 p3 = route.points[i + 1];
...
void EdgeRenderer::computeBezierControls(const Engine::Core::Vec2& p0,
...
void EdgeRenderer::appendBezierSamples(std::vector<Engine::Core::Vec2>& points,
                                       ...
                                       int segments)
{
    ...
    for (int i = 0; i <= segments; ++i)
    {
        ...
        const float x = omt * omt * omt * p0.x + 3.0F * omt * omt * t * p1.x + 3.0F * omt * t * t * p2.x +
                        t * t * t * p3.x;
        const float y = omt * omt * omt * p0.y + 3.0F * omt * omt * t * p1.y + 3.0F * omt * t * t * p2.y +
                        t * t * t * p3.y;
